...
const MapNavigation = () => {
  ...
  const { markersData, mapValues: { positioning: { scale }}, currentMap } = useSelector((state) => state.worldMapStore);

  const hideMarkers = () => {
    dispatch(changeDisplay({ currentMap, display: markersData[currentMap][0].display === 'block' ? 'none' : 'block' }));
  };
  ...
  const scalePlus = async () => {
    ...
    const maxScale = currentMap === 'teyvat' ? 7 : 4;
